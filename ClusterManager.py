...
class ActorResolver:
    ...
    clsSimilarity = MinhashClusterSimilarity()

    compressed_dict = {}

    def __init__(self, clsSimilarity = FuzzyClusterSimilarity()):
        self.clsSimilarity = clsSimilarity
    ...
    def getClosestMatch(self, actor_name):
        max_ratio = self.clsSimilarity.THRESHOLD
        possible_actor = None
    ...
    def compress(self, actor_freq_dict={}):
        ...
        for i in range(0, len(list_of_names)):
            maxMatched = None
            maxRatio = self.clsSimilarity.THRESHOLD
    ...
    def bbn_compress(self, actor_freq_dict={}):
        compressed_dict = {}
        list_of_names = []
        ...
        for key in list_of_names:
            ...
            if parent not in compressed_dict:
                print "Inserting"
                print actor_freq_dict[key]
                compressed_dict[parent] = actor_freq_dict[key]
            else:
                print "Updating"
                #compressed_dict[parent][0].append(key)
                maximum = max(compressed_dict[parent], actor_freq_dict[key])
                compressed_dict[parent] =  maximum
