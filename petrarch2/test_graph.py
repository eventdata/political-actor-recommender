    ...
    for line in input_file:
        ...
        for dict_actor_item in ner_dic.keys():
            ...
            if actor_dict.contains(dict_actor_item):
                existing_actor_list.append(dict_actor_item)
                continue
        ...
        all_nouns = comp_dict.keys()
        for i in range(0, len(all_nouns)):
            name = all_nouns[i]
            neighbours = {}
            if name in interaction_graph:
                neighbours = interaction_graph[name]
            else:
                interaction_graph[name] = neighbours
            for j in range(0, len(all_nouns)):
                if i == j:
                    continue
                else:
                    if all_nouns[j] in neighbours:
                        neighbours[all_nouns[j]] = neighbours[all_nouns[j]] + 1
                    else:
                        neighbours[all_nouns[j]] = 1
...
exist_uf = compress(existing_actor_list)
exist_actor_set = set()

for item in existing_actor_list:
    exist_actor_set.add(exist_uf.find(item))


print 'Exisiting actor found ' ,len(exist_actor_set)
...
label_prop = LabelPropagation(graph)
